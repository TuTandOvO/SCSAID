...
- The inline storage notice explains first-party session cookies, visit-count protection, locally saved interface preferences, and private developer-only visitor analytics. Country is resolved locally from an on-server GeoLite database only when a countable page view is recorded; private analytics may retain a random visitor ID, coarse browser metadata, country, and server-resolved IP address for site administration, but does not use advertising or third-party tracking cookies.
